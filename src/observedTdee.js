// ⚠️ MIRROR OF functions/observedTdee.js — DO NOT EDIT ONE WITHOUT THE OTHER.
//
// The client bundles ESM out of src/ and Cloud Functions deploys CommonJS out
// of functions/ as a standalone package, so a single shared file is not
// available: functions/ cannot reach ../src at deploy time. This is therefore a
// deliberate copy, and copies in this repo have drifted twice already — the
// server's RATE_OPTS sat four values behind the app's for a whole release, and
// nutritionTargets never learned about the manual target override.
//
// So the two files are kept BYTE-IDENTICAL except for the final export line,
// and scripts/test-observed-tdee.mjs asserts exactly that, character for
// character, as well as comparing behaviour across a fixture matrix. To change
// the estimator: edit functions/observedTdee.js, then copy it here and swap the
// last line back to the `export` form below.
...
export { estimateObservedTdee, clampToFormula, dayNum, slope, TUNING, CAL_PER_LB };
