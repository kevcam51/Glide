...
- Logging food: from a description OR a PHOTO (identify items/portions from the image), estimate calories + P/C/F, then call propose_meal to show a tappable Accept/Edit card. Be rigorous about PORTION — when it's ambiguous ('some rice', 'a bowl of pasta', a plate photo), briefly state the serving you're assuming (e.g. '~1 cup / 200g') or ask, so grams aren't a blind guess. Account for INVISIBLE calories — cooking oil, butter, dressings, sauces — and ask if a cooked dish likely has them. The card shows the name + calories + macros, so keep your text reply to ONE short line (e.g. "Here's my estimate — tap to log.") — don't re-list items or repeat macros, even for a long list. The card saves it — do NOT also call log_meal. Ask the meal type if unclear; support corrections ("make it one egg") by proposing again; note photo estimates are approximate. MULTIPLE foods at once (the user lists or photographs several distinct foods/meals to log): do NOT use propose_meal — its card shows only ONE at a time and forces a tap per item. Instead call log_meal DIRECTLY, once per item, ALL IN THE SAME TURN, so every item logs together in one shot with NO tapping. When the user has already given you the list of foods, THAT is your go-ahead — log them right away, do not ask for another confirmation. CRITICAL: to log, you must actually EMIT the log_meal tool calls. Writing "logging all eight now" or "done!" in text logs NOTHING — if you say you logged something, you MUST have called log_meal for it in the same response (and after a client-id lookup, immediately emit the log_meal calls; don't stop after the lookup). After the calls succeed, give a brief one-line-per-item summary, and note they can edit or remove any of it in "Meals & Food Today" — or just tell you to change/remove one and you'll fix it. Reserve the single propose_meal Accept card for when there is exactly ONE food/meal.
...
async function runToolRound(toolUses, toolCtx) {
  ...
  for (const tu of toolUses) {
    ...
    if (["log_meal", "remove_meal", "log_workout", "log_weigh_in", "log_check_in", "log_measurements", "log_water", "set_targets", "set_workout_schedule", "set_personal_info", "create_plan", "switch_plan", "rename_plan", "set_notification_prefs", "add_custom_exercise"].includes(tu.name) && out && out.ok) wrote = true;
